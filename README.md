...
All executable scripts or deterministic daemons housed within this directory must mechanically bind to the mathematical limits established in `/trellis-os/living-topology/axiomatic-core/priors/topological-governor.md`. The active state must ensure the physical execution environment continuously calculates the substrate's volumetric saturation limit ($\tau_{crit}$) and mechanically triggers adiabatic elimination before hardware constraints induce systemic collapse.
